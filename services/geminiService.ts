...
export const checkPlagiarism = async (text: string): Promise<{ text: string, sources: any[] }> => {
  try {
    const response = await ai.models.generateContent({
      model: 'gemini-2.5-flash',
      contents: `Please check the following text for potential plagiarism against public web sources. Analyze the text and identify any sentences or phrases that are highly similar to existing online content. For each match you find, explicitly mention it. At the end, provide a brief overall assessment. Text: "${text}"`,
      config: {
        systemInstruction: "You are a plagiarism detection assistant. Your role is to compare a given text against Google Search results and identify potential matches. You must present the source URLs provided by the tool.",
        tools: [{ googleSearch: {} }],
      },
    });
    ...
    return { text: resultText, sources };
  } catch (error) {
    console.error("Error checking plagiarism:", error);
    return {
      text: "Failed to check for plagiarism. The API may be experiencing issues or the content may have been blocked. Please check the console for details.",
      sources: []
    };
  }
};
...
export const getStudyTip = async (prompt: string): Promise<string> => {
  try {
    const response = await ai.models.generateContent({
        model: 'gemini-2.5-flash',
        contents: `A student asked for study advice. Their question is: "${prompt}". Provide a concise, helpful, and encouraging response in 2-3 short paragraphs. Use markdown for formatting if needed (e.g., lists).`,
        config: {
            systemInstruction: "You are a friendly and encouraging AI study coach. You provide practical, actionable tips to students."
        }
    });
    return response.text;
  } catch (error) {
