...
const ConceptExplainer: React.FC<ConceptExplainerProps> = ({ currentUser, navigateTo, isSandboxMode }) => {
  const [concept, setConcept] = useState('');
  const [explanation, setExplanation] = useState('');
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState('');

  const { usage, isLimitReached, incrementUsage } = useUsageTracker(currentUser, 'explainer', DAILY_LIMIT, isSandboxMode);
  ...
  }, [concept, isFreeTier, isLimitReached, incrementUsage, isSandboxMode]);
  ...
  return (
    <div className="p-8 h-full flex flex-col">
      ...
      <div className="flex items-start gap-4 mb-6">
        ...
        />
        <Button onClick={handleExplain} disabled={isLoading || !concept || (isFreeTier && isLimitReached && !isSandboxMode)}>
