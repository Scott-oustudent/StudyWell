...
const Scheduler: React.FC<SchedulerProps> = ({ isSandboxMode, events, setEvents }) => {
  ...
  const monthlyEventsForExport = useMemo(() => {
    const year = currentDate.getFullYear();
    const month = currentDate.getMonth();

    const monthlyEventsList = Object.entries(eventsByDate)
      .filter(([dateStr]) => {
        const eventDate = new Date(dateStr + 'T00:00:00');
        return eventDate.getFullYear() === year && eventDate.getMonth() === month;
      })
      .flatMap(([, eventsOnDay]) => eventsOnDay);
    
    return monthlyEventsList.sort((a, b) => new Date(`${a.date}T${a.time}`).getTime() - new Date(`${b.date}T${b.time}`).getTime());
  }, [eventsByDate, currentDate]);
  
  const monthlyAgendaTextContent = useMemo(() => {
    if (monthlyEventsForExport.length === 0) {
        return "No events for this month.";
    }

    const groupedByDate = monthlyEventsForExport.reduce((acc, event) => {
      (acc[event.date] = acc[event.date] || []).push(event);
      return acc;
    }, {} as Record<string, CalendarEvent[]>);

    let content = `Agenda for ${currentDate.toLocaleString('default', { month: 'long', year: 'numeric' })}\n\n`;

    Object.entries(groupedByDate).forEach(([dateStr, eventsOnDay]) => {
        const date = new Date(dateStr + 'T00:00:00');
        content += `--- ${date.toLocaleDateString('default', { weekday: 'long', month: 'long', day: 'numeric' })} ---\n`;
        eventsOnDay.forEach(event => {
            content += `${formatTime12hr(event.time)} - ${event.type.charAt(0).toUpperCase() + event.type.slice(1)}: ${event.title}\n`;
        });
        content += '\n';
    });

    return content;
  }, [monthlyEventsForExport, currentDate]);
  ...
  return (
    <div className="p-8 h-full flex flex-col">
        ...
        <Card className="p-6 lg:col-span-2 flex flex-col">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-2">
              <ExportMenu
                elementId="monthly-agenda-export"
                textContent={monthlyAgendaTextContent}
                filename={`agenda-${currentDate.getFullYear()}-${String(currentDate.getMonth() + 1).padStart(2, '0')}`}
                disabled={monthlyEventsForExport.length === 0}
              />
        ...
        <div className="flex flex-col gap-6">
          <Card className="p-6 flex-grow flex flex-col">
                ...
                <ul id="agenda-export-list" className="space-y-3">
                  {selectedDayEvents.map(event => (
                    <li key={event.id} className={`p-3 rounded-lg flex justify-between items-start border-l-4 ${EVENT_TYPE_STYLES[event.type].bg} ${EVENT_TYPE_STYLES[event.type].border}`}>
                      ...
                      <button onClick={() => handleDeleteEvent(event.id)} className={`text-gray-400 hover:text-red-600 transition-colors`} aria-label="Delete event" disabled={isSandboxMode}>
            ...
            <div className="mt-4 pt-4 border-t">
              <Button onClick={() => setIsModalOpen(true)} className="w-full" disabled={isSandboxMode}>Add New Event</Button>
            </div>
          </Card>
        </div>
      </div>
      ...
      <div id="monthly-agenda-export" className="hidden" aria-hidden="true">
        <h2 style={{ fontSize: '24px', fontWeight: 'bold' }}>Agenda for {currentDate.toLocaleString('default', { month: 'long', year: 'numeric' })}</h2>
        <br />
        {Object.entries(
            monthlyEventsForExport.reduce((acc, event) => {
                (acc[event.date] = acc[event.date] || []).push(event);
                return acc;
            }, {} as Record<string, CalendarEvent[]>)
        ).map(([dateStr, eventsOnDay]) => (
            <div key={dateStr} style={{ marginBottom: '16px' }}>
                <h3 style={{ fontSize: '18px', fontWeight: 'bold', borderBottom: '1px solid #ccc', paddingBottom: '4px', marginBottom: '8px' }}>
                    {new Date(dateStr + 'T00:00:00').toLocaleDateString('default', { weekday: 'long', month: 'long', day: 'numeric' })}
                </h3>
                <ul style={{ listStyle: 'none', paddingLeft: '0' }}>
                    {eventsOnDay.map(event => (
                        <li key={event.id} style={{ marginBottom: '4px' }}>
                            <span style={{ fontWeight: 'bold' }}>{formatTime12hr(event.time)}</span> - <span style={{ textTransform: 'capitalize' }}>{event.type}</span>: {event.title}
                        </li>
                    ))}
                </ul>
            </div>
        ))}
        {monthlyEventsForExport.length === 0 && <p>No events for this month.</p>}
      </div>
    </div>
  );
};
