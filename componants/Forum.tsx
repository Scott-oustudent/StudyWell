...
const Forum: React.FC<ForumProps> = ({ currentUser, users, setUsers, logAuditAction, createNotification, isSandboxMode, forumPosts, setForumPosts, forumCategories, setForumCategories }) => {
  ...
  const filteredPosts = useMemo(() => {
    return (selectedCategoryId ? forumPosts.filter(p => p.categoryId === selectedCategoryId) : forumPosts)
        .sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
  }, [forumPosts, selectedCategoryId]);
  ...
  const handleRequestCategory = async () => {
    ...
    if (!isSandboxMode) {
        logAuditAction(currentUser.id, currentUser.name, AuditActionType.MESSAGE_SENT, `Requested new forum category: "${newCategoryName}"`);
    }
  ...
  const handleDeletePost = (post: ForumPost) => {
    ...
    if (window.confirm('Are you sure you want to delete this post? This action cannot be undone.')) {
        setForumPosts(prevPosts => prevPosts.filter(p => p.id !== post.id));
  ...
  const onModerateClose = useCallback(() => {
  ...
  return (
    ...
    <Card className="p-6 h-full flex flex-col md:flex-row gap-6">
      ...
      <aside className="md:w-1/4 flex-shrink-0">
        ...
        <Button onClick={() => setIsRequestModalOpen(true)} variant="secondary" className="w-full mt-6" disabled={isSandboxMode}>
          Request a Category
        </Button>
      </aside>
      ...
      <main className="flex-1 overflow-y-auto">
        <div className="flex justify-between items-center mb-4">
            <h2 className="text-2xl font-bold text-gray-800">
                {selectedCategoryId ? forumCategories.find(c => c.id === selectedCategoryId)?.name : 'All Posts'}
            </h2>
                ...
                <div className="absolute top-2 right-2 flex gap-2">
                  ...
                  {canModerate && (
                      <button 
                          ...
                          disabled={isSandboxMode}
                      >
                          <Icon><svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></Icon>
                      </button>
                  )}
                </div>
                <span className="text-xs font-semibold bg-purple-100 text-purple-800 py-1 px-2 rounded-full">
                  {forumCategories.find(c => c.id === post.categoryId)?.name || 'Uncategorized'}
                </span>
