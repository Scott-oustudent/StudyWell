...
import { User, AppView, SubscriptionTier, SavedCitation } from '../types';
...
interface CitationWizardProps {
  ...
  setSavedCitations: React.Dispatch<React.SetStateAction<SavedCitation[]>>;
}

const CITATION_LIMIT = 10;

const CitationWizard: React.FC<CitationWizardProps> = ({ currentUser, navigateTo, isSandboxMode, savedCitations, setSavedCitations }) => {
  ...
  const isCitationLimitReached = isFreeTier && savedCitations.length >= CITATION_LIMIT && !isSandboxMode;
  ...
  const handleSaveCitation = useCallback(() => {
    ...
    setGeneratedCitation('');
    setSuccessMessage(isSandboxMode ? 'Save disabled in Sandbox Mode' : 'Citation saved!');
    setTimeout(() => setSuccessMessage(''), 2000);
  }, [generatedCitation, style, sourceType, setSavedCitations, isCitationLimitReached, isSandboxMode]);

  const handleDeleteCitation = useCallback((id: number) => {
    if (isSandboxMode) {
        setSuccessMessage('Delete disabled in Sandbox Mode');
        setTimeout(() => setSuccessMessage(''), 2000);
        return;
    };
    setSavedCitations(prev => prev.filter(c => c.id !== id));
  }, [setSavedCitations, isSandboxMode]);
              ...
              <Card className="p-6 flex-grow flex flex-col">
                ...
                <div className="mt-4 flex justify-end items-center gap-4">
                    ...
                    <Button onClick={() => handleCopyToClipboard(generatedCitation)} variant="secondary" className="flex items-center gap-2">
                     ...
                     <Button onClick={handleSaveCitation} variant="primary" className="flex items-center gap-2" disabled={isCitationLimitReached && !isSandboxMode}>
                ...
                <li key={c.id} className="bg-gray-50 p-4 rounded-lg border border-gray-200">
                    ...
                    <div className="mt-3 flex justify-between items-center">
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <button onClick={() => handleDeleteCitation(c.id)} className="text-gray-500 hover:text-red-600 transition-colors" title="Delete citation" disabled={isSandboxMode}>
