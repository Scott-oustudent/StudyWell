...
const StudentWellness: React.FC<StudentWellnessProps> = ({ currentUser, logAuditAction, isSandboxMode, wellness, setWellness }) => {
  ...
  const { videos, tracks, articles } = wellness;
  const { setVideos, setTracks, setArticles } = setWellness;

  const canManageContent = useMemo(() => {
    return !isSandboxMode && [UserRole.STAFF, UserRole.ADMIN].includes(currentUser.role);
  }, [currentUser.role, isSandboxMode]);
