...
const NoteSummarizer: React.FC<NoteSummarizerProps> = ({ currentUser, navigateTo, isSandboxMode }) => {
  const [notes, setNotes] = useState('');
  const [summary, setSummary] = useState('');
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState('');
  
  const { usage, isLimitReached, incrementUsage } = useUsageTracker(currentUser, 'summarizer', DAILY_LIMIT, isSandboxMode);
  ...
  }, [notes, isFreeTier, isLimitReached, incrementUsage, isSandboxMode]);
    ...
    <div className="p-8 h-full flex flex-col">
      ...
      <div className="flex-grow flex flex-col gap-6">
        ...
        <div className="flex items-center">
          <Button onClick={handleSummarize} disabled={isLoading || !notes || (isFreeTier && isLimitReached && !isSandboxMode)}>
