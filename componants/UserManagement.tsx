    ...
    const handleConfirmResetPassword = () => {
        if (!userToReset) return;
         ...
         setUsers(prevUsers => prevUsers.map(user => 
            user.id === userToReset.id ? { ...user, isPasswordTemporary: true } : user
         ));
         logAuditAction(currentUser.id, currentUser.name, AuditActionType.USER_PASSWORD_RESET, `Reset password for ${userToReset.name}.`);
         createNotification(userToReset.id, `Your password has been reset by an administrator. You will be required to set a new password on your next login.`, NotificationType.WARNING);
         alert(`Password for ${userToReset.name} has been reset. They will be prompted to change it at next login.`);
         setUserToReset(null);
    };
                            ...
                            <tr key={user.id}>
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <div className="flex items-center flex-wrap gap-2">
                                        ...
                                        <Button variant="secondary" onClick={() => openResetPasswordModal(user)} className="text-xs !px-2 !py-1" disabled={user.id === currentUser.id}>Reset Pass</Button>
