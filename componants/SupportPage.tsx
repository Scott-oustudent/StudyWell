...
const SupportPage: React.FC<SupportPageProps> = (props) => {
  ...
  const handleSuggestionSubmit = (e: React.FormEvent) => {
    ...
    if (!suggestionSubject.trim() || !suggestionBody.trim()) {
      setSuggestionError('Please provide both a title and a description for your suggestion.');
      return;
    }
    setSuggestionError('');
    addStaffMessage({
      senderId: currentUser.id,
      senderName: currentUser.name,
      subject: `Feature Suggestion: ${suggestionSubject}`,
      body: suggestionBody,
      isRead: false,
    });
  ...
  return (
    <div className="p-8 h-full flex flex-col bg-gray-50">
      ...
      <p className="text-lg text-gray-600 mb-6">Find answers, suggest new features, and get help with any issues.</p>
      ...
      <div className="flex-grow grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        ...
        <div className="space-y-8">
            ...
            {/* Suggest a Feature */}
            <Card className="p-6">
                 <h2 className="text-2xl font-bold text-gray-800 mb-4">Suggest a Feature</h2>
                 {suggestionSuccess ? (
                    <div className="text-center text-green-700 bg-green-100 p-4 rounded-lg">
                        <p className="font-semibold">Suggestion Submitted!</p>
                        <p className="text-sm">Thank you for your feedback. Our team will review your idea.</p>
                    </div>
                 ) : (
                    <form onSubmit={handleSuggestionSubmit} className="space-y-4">
                        <div>
                            <label htmlFor="suggestion-subject" className="block text-sm font-medium text-gray-700">Suggestion Title</label>
                            <input id="suggestion-subject" type="text" value={suggestionSubject} onChange={e => setSuggestionSubject(e.target.value)} className="mt-1 w-full p-2 border border-gray-300 rounded-md shadow-sm" placeholder="e.g., Dark mode for the app" required />
                        </div>
                        <div>
                            <label htmlFor="suggestion-body" className="block text-sm font-medium text-gray-700">Description</label>
                            <textarea id="suggestion-body" rows={5} value={suggestionBody} onChange={e => setSuggestionBody(e.target.value)} className="mt-1 w-full p-2 border border-gray-300 rounded-md shadow-sm" placeholder="Describe your idea and why it would be helpful..." required />
                        </div>
