...
type EssayTool = 'thesis' | 'outline' | 'expander' | 'proofreader';
const DAILY_LIMIT = 2;
...
const EssayHelper: React.FC<EssayHelperProps> = ({ currentUser, navigateTo }) => {
  ...
  const { usage, isLimitReached, incrementUsage } = useUsageTracker(`essay-${activeTool}`, DAILY_LIMIT);
    ...
    try {
      ...
      switch (activeTool) {
        case 'thesis':
          ...
          response = await geminiService.generateThesis(topic);
          break;
        case 'outline':
          if (!topic || !thesis) { setError('Please enter a topic and a thesis statement.'); break; }
          response = await geminiService.generateOutline(topic, thesis);
          break;
        case 'expander':
          if (!pointToExpand) { setError('Please enter a point to expand.'); break; }
          response = await geminiService.expandPoint(pointToExpand);
          break;
        case 'proofreader':
          if (!textToProofread) { setError('Please enter text to proofread.'); break; }
          response = await geminiService.proofreadText(textToProofread);
          break;
      }
      setResult(response);
      if(response && isFreeTier) {
        incrementUsage();
      }
    } catch (err) {
      ...
      <div className="flex-grow grid grid-cols-1 md:grid-cols-2 gap-8">
        <Card className="p-6 flex flex-col">
          ...
          {renderToolContent()}
           {renderUsageInfo()}
          <div className="mt-6">
            <Button onClick={handleGenerate} disabled={isLoading || (isFreeTier && isLimitReached)} className="w-full">
